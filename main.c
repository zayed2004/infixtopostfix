...
/* define function that is used to determine whether any symbol is operator or not (that is
symbol is operand)
...
push(x);
/* because just above while loop will terminate we have poped one extra item for which
condition fails and loop terminates, so that one*/
push(item);
}
